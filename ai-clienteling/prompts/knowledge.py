...
# 진단 요청 감지 (2026-08-19 추가).
#
# 위 규칙("상태를 말했을 때만")에 구멍이 있었다 — 고객이 상태 단어 없이
# "Pina는 케어를 받는 편이 좋을까요?" 라고 **판단을 요청**하면 컨디션이 계속
# 가려져서, 카드에는 소견이 떠 있는데 말은 "상태에 따라 다를 수 있습니다" 로
# 회피했다 (C010 손 테스트). 요청받은 평가는 케어지 감시가 아니다.
#
# 정책 질문과 판단 요청을 가른다:
#   · "수선 되나요 / 수선이 어렵다던데요" — 정책 질문. 서비스 데이터로 답하면
#     되고 물건 평가가 필요 없다. 열지 않는다 (옛 낭독 사고가 이 형태였다).
#   · "케어를 받는 편이 좋을까요 / 케어를 해도 괜찮아질까요" — 답 자체가 평가라
#     데이터 없이는 빈말이 된다. 연다.
# 가르는 신호는 케어 단어와 **판단 어미가 같은 발화**에 있는가이다.
# "될까"는 넣지 않는다 — "수선 될까요?" 는 가능 여부(정책) 질문이다.
# 목록이라 새겠지만 새는 방향이 "안 열림"(현행 유지)이라 감시 쪽으로는 안 넘어진다.
CARE_JUDGMENT_SERVICES = ("케어", "수선", "손질", "관리")
CARE_JUDGMENT_HINTS = (
    "좋을까", "나을까", "괜찮아질까", "괜찮을까", "필요할까",
    "해야 할까", "해야할까", "받아야 할까", "받아야할까",
)


# 케어 상담이 이미 열려 있는가 — 고객 발화가 아니라 **우리 발화**로 판정한다
# (2026-08-19). 위의 단어 목록들은 고객 발화를 보므로 표현 변형("받아야
# 될까요", "심각한가요"…)마다 샌다. 고객의 말은 무한해서 다 넣을 수 없다.
# 어드바이저 발화는 우리가 만든 출력이라 어휘가 좁다 — 우리가 보유 제품의
# 케어를 이미 화제로 올렸다면, 그 뒤 고객이 어떤 표현으로 묻든 소견으로
# 답하는 것은 요청받은 평가지 감시가 아니다. 고객 발화 목록은 "고객이 먼저
# 케어를 여는 첫 턴" 커버용 보조로만 남는다. 첫 턴을 놓쳐도 우리 답변이
# 케어를 언급하는 순간 다음 턴부터 열린다 — 한 턴 만에 스스로 치유된다.
#
# "접수"·"A/S" 는 넣지 않는다 — 접수는 재고 보관 요청에도 쓰이고,
# A/S 는 정책 안내 턴(수선 여부를 묻기만 한 대화)까지 열어버린다.
# "살펴" 가 아니라 "살펴드" 다 (2026-08-19 손 테스트) — "다른 라인을
# 살펴보시는 것도 좋겠습니다"(제품 구경)가 걸려 구매 턴에 마모 카드가 남았다.
# 케어 용법은 존대 보조동사가 붙은 "살펴드릴까요/살펴드리겠습니다" 형태라
# "살펴드" 로 구경과 갈린다.
ADVISOR_CARE_WORDS = ("케어", "수선", "클리닝", "점검", "살펴드", "손질", "봐드렸")


def care_thread_open(conversation_history, owned_products) -> bool:
    """우리(어드바이저)가 이 고객의 보유 제품 케어를 이미 화제로 올렸는가.

    어드바이저 턴에 케어 단어와 보유 제품 이름(첫 토큰)이 **같은 턴**에
    있어야 한다 — 케어 단어만으로 열면 "케어 서비스도 있습니다" 같은
    지나가는 안내로 열리고, 이름만으로 열면 실루엣 비교 턴으로 열린다.
    """
    tokens = [
        (p.get("name") or " ").split()[0]
        for p in owned_products or []
    ...
    for turn in conversation_history or []:
        ...
        if any(w in text for w in ADVISOR_CARE_WORDS) and any(
...
def asks_care_judgment(user_texts) -> bool:
    """고객이 자기 물건의 케어 여부에 대한 판단을 요청했는가.

    발화 단위로 본다 — 케어 단어와 판단 어미가 다른 턴에 흩어져 있으면
    (예: 1턴 "수선 되나요?" + 3턴 "어느 게 좋을까요?") 요청이 아니다.

    "어떻게"가 있으면 방법 질문이다 ("관리를 어떻게 하면 좋을까요?") —
    케어 여부가 아니라 방법을 물은 것이라 열지 않는다. 방법 질문에 마모를
    먼저 꺼낸 것은 기록된 사고 형태다 ("핸들에 마모가 보이는데").
    """
    for text in user_texts or []:
        if "어떻게" in text:
            continue
        if any(s in text for s in CARE_JUDGMENT_SERVICES) and any(
            j in text for j in CARE_JUDGMENT_HINTS
        ):
            return True
    return False
