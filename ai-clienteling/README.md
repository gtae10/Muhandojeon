...
콘솔(`agent.py`)은 MCM 데모 세계의 고객 10명을 씁니다.
...
| C006 | 3년 전 케어 이력 있음 | 지난 케어를 관계로 꺼내기, 겹치는 보유를 언급하고 용도 여쭙기 |
| C007 | 부산 거주 | 지역을 단정하지 않고 여쭙기 — 말씀하신 도시가 아는 곳이면 이름을, 모르는 곳이면 확인 제안 |
| C008 | 싱가포르 거주 · 영어 | 다국어 — 응답 언어를 코드가 정함 |
| C009 | 결제 완료, 수령 방법 미정 | 배송/픽업 안내 (`delivery` 액션) |
| C010 | 보유 3점 · 7년 사용 제품 | 교체 요구를 케어로 받기, 진단 요청("케어를 받아야 할까요?")에 소견으로 답하기 |

시연에서 보여줄 네 대화는 `tests/test_rehearsal.py` 에 그대로 들어 있습니다
(C001 · C006 · C010 · C007). 왜 이 10가지인지는 [DESIGN.md](DESIGN.md) 1절에 있습니다.

**통합 화면(`/preview`)에서는 여기에 팀 fixture 고객 4명이 더해져 14종입니다**
("한 메종" 세계관 — C 고객도 같은 화면에서 자산 근거 카드와 함께 돕니다).
계기(장바구니·케어 시점)가 있는 고객만 시나리오 목록에 올립니다 — 계기가 곧
시나리오라서, 계기 없는 CU-0005·0006 은 목록에 없습니다.

| ID | 상황 | 여기서 보이는 것 |
|---|---|---|
| CU-0001 | 장바구니 이탈 (Solène) · 보유 5점, 핸들 마모 자산 포함 | fixture 제품을 사실로 상담, 케어 화제로 이어지면 근거 카드 |
| CU-0002 | 컨디션 점검 1개월 경과 | 케어 오프닝 + 근거 카드 (컨디션 소견) |
| CU-0003 | 장바구니 이탈 (Aurelia Oxford 구두) | **피팅 시나리오** — 사이즈 상담 → 피팅 제안·수락 시 `cta=BOOK_FITTING` |
| CU-0004 | 장바구니 이탈 (Solène) · 소품만 보유 | 가방 첫 구매 상담 |
...
data/
  ...
  integration_*.json  통합 경로 데이터 5종 — 스크립트 산출물, 직접 수정 금지
                      (카탈로그 18종 · 고객 16명 · 확장 재고 · 고객별 자산 ·
                       서비스 정책 = MCM 정책 + 할인 정책 한 절)
tests/              회귀 테스트 13종
```
